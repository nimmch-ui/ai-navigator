...
- **Dynamic Rerouting & ETA Optimization (Q6-A2 Part 2/3 Complete):** Smart ETA calculation and automatic rerouting system that responds to traffic conditions:
  - **SmartETA Engine (services/navigation/SmartETA.ts):** Traffic-aware ETA calculation with computeETA() factoring in real-time congestion (0-100 scale with exponential delay curves), weather conditions (rain +5-15%, snow +10-30%, low visibility +5-15%), stop-and-go patterns (medium congestion 50-80% adds 1.5s/point, severe 80-100% adds 3s/point), UI mode adjustments (ECO +10% time); recalculateETAOnUpdate() responds to ai:trafficUpdate events; returns ETAResult with estimated seconds, confidence (0-1), and breakdown (baseTime, trafficDelay, weatherDelay, stopAndGoDelay); setupETAAutoUpdate() provides auto-recalculation on traffic updates
  - **RoutingController (services/navigation/RoutingController.ts):** Dynamic rerouting logic with startMonitoring() watching ai:trafficUpdate events and route progress; evaluateReroute() checks congestion ahead (60%+ threshold), severe incidents (high/severe severity), and multiple medium incidents (2+); queries alternative routes via Mapbox Directions API; compares routes with RerouteProposal containing timeSaved, distanceImpact, ecoImpact, reason, severity; thresholds: minTimeSaved 180s (3 min), maxDistanceIncrease 5km; supports auto-reroute mode (auto-accept) or manual mode (user approval); emits route:evaluating_alternatives, route:reroute_proposal, route:reroute_accepted events
  - **Voice Integration:** Voice prompts via voiceGuidanceService: "Traffic ahead, recalculating best route" when evaluating alternatives; "New route available, X minutes faster" when proposal found; priority-based announcements (high for severe incidents, normal for congestion)
  - **HUD Components:** TrafficAlertBanner displays traffic conditions with severity colors (yellow/orange/red), congestion percentage, incident count, dismissible; RerouteProposalCard shows time saved badge, reason, distance impact (green/orange arrows), eco impact, Accept/Keep Current buttons with data-testids for testing
  - **User Settings:** RerouteSettings in preferences.ts with autoAccept (auto vs manual approval), minTimeSavingsMinutes (default 2), etaIncreaseThresholdPercent (default 15%), offRouteDistanceMeters (default 100m)
  - **Next Steps (Part 3/3):** Map visualization with color-coded route segments showing congestion levels; incident icons with popups; full navigation loop integration; live ETA updates in UI

...
import {
  detectCamerasOnRoute,
  getCurrentSpeedLimit,
  type CameraProximityWarning,
} from "@/services/cameraProximity";
import {
  calculateEcoEstimate,
  type EcoEstimate,
} from "@/services/ecoEstimates";
import {
  fetchWeather,
...
export default function Home() {
  ...
  const [mapCenter, setMapCenter] =
    useState<[number, number]>(getZurichFallback());
  ...
  const {
    ...
    setAmbientMusic,
    hapticsEnabled,
    setHapticsEnabled: setHapticsEnabledPref,
  } = useUiMode();
  ...
  const [routePreference, setRoutePreference] =
    useState<RoutePreference>("fastest");
  ...
  >([]);
  const [dismissedCameraIds, setDismissedCameraIds] = useState<Set<string>>(
    new Set(),
  );
  const [currentSpeedLimit, setCurrentSpeedLimit] = useState<number | null>(
    null,
  );
  ...
  const [rerouteSettings, setRerouteSettings] = useState(
    PreferencesService.getPreferences().rerouteSettings,
  );
  ...
  const [region, setRegion] = useState<Region>(
    PreferencesService.getPreferences().region,
  );
  ...
  const [laneSegments, setLaneSegments] = useState<Map<number, LaneSegment>>(
    new Map(),
  );
  const [currentNavigationPosition, setCurrentNavigationPosition] = useState<
  ...
  //     console.log('[Home] ✅ DEV demo navigation loaded');
  //   }
  // }, []);
  ...
  const handleRadarToggle = () => {
    ...
    toast({
      ...
      description: newValue
        ? "Live weather radar overlay enabled"
        : "Weather radar overlay disabled",
    });
  };
  ...
  const geocodeAddress = useCallback(
    async (query: string) => {
      ...
      const results = await geocodeFetcher.current.fetch<GeocodingResult[]>(
        async (signal) => {
  ...
  const handleResultSelect = (result: SearchResult) => {
    ...
    const validatedCoords = validateCoordinates(
      result.coordinates,
    ...
    setMapZoom(15);
    setMarkers([
      { lat: validatedCoords[0], lng: validatedCoords[1], label: result.name },
    ]);
    setSearchResults([]);
  ...
  const handleCalculateRoute = useCallback(async () => {
    ...
    const result = await routeFetcher.current.fetch<RouteResult>(
      async (signal) => {
  ...
  const activeWarning = cameraWarnings.find(
    (w) => !dismissedCameraIds.has(w.camera.id),
  );
  ...
  const isDarkMode =
    mapTheme === "night" ||
    (mapTheme === "auto" &&
      resolveTheme(mapTheme, mapCenter[0], mapCenter[1]) === "night");
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      ...
      const nearbyCameras = cameraWarnings.map(
        (w) =>
      ...
      const weatherStatus =
        weatherData.length > 0
          ? weatherData
              .map(
                (w) =>
                  `${w.location}: ${w.description}, ${Math.round(w.temperature)}°C`,
              )
              .join("; ")
          : undefined;
      ...
      if (gpsService.isGPSSupported()) {
        ...
        const unsubscribeError = gpsService.onError((error) => {
          ...
          if (error.code === error.PERMISSION_DENIED) {
            toast({
              ...
              description:
                "Please enable location permissions to use real-time navigation. Using simulated route instead.",
              variant: "destructive",
            });
            ...
            toast({
              ...
              description:
                "Unable to get your location. Using simulated navigation.",
            });
          }
        });
        ...
        fallbackTimer = setTimeout(() => {
          if (!hasReceivedPosition) {
            if (import.meta.env.DEV) {
              console.warn(
                "[GPS] No position received after 15s, falling back to simulation",
              );
            }
            toast({
        ...
        if (import.meta.env.DEV) {
          console.warn(
            "[GPS] GPS not supported, using demo position simulation",
          );
        }
        let currentIndex = 0;
  ...
  const handleStartNavigation = async () => {
    if (
      ...
      if (voiceEnabled && routeResult.steps.length > 0) {
        ...
        if (secondStep && secondStep.distance > 100) {
          setTimeout(() => {
            ...
            announce(`In ${distance} meters, ${secondStep.instruction}`, {
              priority: "normal",
            });
          }, 8000);
        }
      ...
      try {
        ...
        console.log("[Weather] Weather data loaded:", {
          originWeather,
          destWeather,
        });
      } catch (error) {
  ...
  const handleModeToggle = useCallback(() => {
    ...
    const newMode =
      currentMode === UiMode.CLASSIC ? UiMode.THREED : UiMode.CLASSIC;
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex">
        ...
        {uiMode !== UiMode.AR && (
          <>
            ...
            <div className="absolute top-20 left-4 right-4 z-20 max-w-md mx-auto space-y-2">
              {!severeWeatherDismissed &&
                weatherData.length > 0 &&
                getSevereWeatherWarning(weatherData) && (
                  <SevereWeatherAlert
            ...
            <div className="absolute bottom-6 right-6 z-30 flex flex-col gap-2">
              ...
              >
                ...
                ) : resolveTheme(mapTheme, mapCenter[0], mapCenter[1]) ===
                  "day" ? (
                  <Sun className="h-5 w-5" />
                ) : (
                  <Moon className="h-5 w-5" />
              ...
              <MapControls
                ...
                onLayersToggle={() =>
                  toast({
                    title: "Layers",
                    description: "Layer toggle coming soon",
                  })
                }
                onCurrentLocation={() => {
            ...
            {(tripEstimate || ecoEstimate || weatherData.length > 0) && (
              <div className="absolute bottom-32 left-4 z-20 max-w-sm space-y-2">
                ...
                {ecoEstimate && uiMode === UiMode.ECO && (
                  <EcoSummary estimate={ecoEstimate} />
                )}
                {tripEstimate && (
                  <TripSummary
            ...
            {selectedReport && (
              <div
                ...
                data-testid="selected-report-popup"
              >
            ...
            {communityReports.length > 0 && !selectedReport && (
              <div
                className="absolute top-32 right-4 z-20 max-h-64 overflow-y-auto"
                data-car-mode-hide
              >
      ...
      <div
        className={`${showChat ? "fixed inset-0 z-[100] lg:relative lg:inset-auto" : "hidden"} lg:flex lg:w-96`}
        data-car-mode-hide
      >
        <Suspense
          fallback={
      ...
      {isARActive && routeResult && routeResult.steps.length > 0 && (
        <ARPreviewOverlay
          ...
          direction={
            routeResult.steps[0].maneuver.modifier?.includes("left")
              ? "left"
              : routeResult.steps[0].maneuver.modifier?.includes("right")
                ? "right"
                : "straight"
          }

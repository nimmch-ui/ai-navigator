...
export interface EventPayloadMap {
  ...
  'sync:device_paired': { userId: string; recordsPushed: number; recordsPulled: number };
  'auth:session_restored': { userId: string };
  'auth:login': { userId: string; method: string };
  'auth:login_failed': { error: string };
  'auth:logout': { userId: string };
  'favorite:added': { favoriteId: string; label: string };
  'favorite:updated': { favoriteId: string };
  'favorite:deleted': { favoriteId: string };
  'route:completed': { distanceKm: number; durationSec: number };
  'trip:saved': { tripId: string };
  'mode:changed': { from: string; to: string };
  'ui:mode_changed': { mode: string };
  'settings:changed': { key: string; value: any };
  'preferences:updated': { keys: string[] };
